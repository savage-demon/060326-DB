...
### 1.5 Неявное преобразование `Не булевы значения` → `BOOLEAN`
...
-- REAL ------------------------------------
SELECT IF(0.0, 'TRUE', 'FALSE');    -- FALSE
SELECT IF(1.0, 'TRUE', 'FALSE');    -- TRUE
-- VARCHAR ---------------------------------
SELECT IF('', 'TRUE', 'FALSE');     -- FALSE
SELECT IF('abc', 'TRUE', 'FALSE');  -- FALSE
-- NULL ------------------------------------
SELECT IF(NULL, 'TRUE', 'FALSE');   -- FALSE
```
⚠️ **ВАЖНО**

Будьте аккуратны с `NULL`❗  
Это "третье состояние" (`UNKNOWN`), но:  

* в `IF()` в MySQL
  *  `NULL` трактуется как `FALSE`
* в обычных SQL-выражениях (`=`, `AND`, `WHERE`)
  * `NULL` остаётся `NULL`
