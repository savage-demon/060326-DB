...
| [**Управление значениями NULL**](#управление-значениями-null)         | Работа со значениям `NULL`: замена на строковые значения (условная и безусловная )        |
...
Другое название: Функции управления потоком (Flow Control Functions)
(в смысле: потоком данных внутри выражения)
```
...
SELECT COALESCE('Hello', 'World'); -- Hello
SELECT COALESCE(NULL, NULL, 123, 456); -- 123
```

---

```
IFNULL(expr1, expr2) - если expr1 IS NULL возвращает второе значение, иначе - первое
```
...
По сути, это **частный случай COALESCE** для 2 аргументов.

---

```
IFNULL(expr1, expr2) - Возвращает `NULL`, если два значения равны, иначе первое значение.
```
...
👉 Часто используют, чтобы **избежать деления на 0**:
...
| Функция    | Что делает         | Кол-во аргументов |
| ---------- | ------------------ | ----------------- |
| `COALESCE` | Первый НЕ-NULL     | 2+                |
| `IFNULL`   | Если NULL → замена | ровно 2           |
| `NULLIF`   | Если равны → NULL  | ровно 2           |


Дополнительная информация: [https://dev.mysql.com/doc/refman/8.4/en/flow-control-functions.html](https://dev.mysql.com/doc/refman/8.4/en/flow-control-functions.html)
